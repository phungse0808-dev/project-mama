  ...
  const header = (
    <>
      <h2 className="panel-title">
        โอกาสฝนตกวันนี้
        {/* จำนวนปีอ่านจากคำตอบของเซิร์ฟเวอร์ ไม่เขียนตัวเลขตายตัวไว้ที่หน้าเว็บ
            เพราะข้อมูลย้อนหลังเพิ่มขึ้นทุกปี ถ้าเขียนไว้จะกลายเป็นตัวเลขผิดเงียบ ๆ

            ชื่อแหล่งข้อมูลอยู่ตรงนี้ ไม่ได้อยู่ในคำอธิบายท้ายแผงเหมือนเดิม
            เพราะย่อหน้าท้ายแผงถูกเอาออกไปแล้ว แต่ต้นทางของตัวเลขยังต้องบอก */}
        <span className="panel-hint">
          คิดจากสถิติย้อนหลัง{data?.years?.length ? ` ${data.years.length} ปี` : ""}ของ NASA POWER ·
          ไม่ใช่การพยากรณ์
        </span>
      </h2>
      ...
      <div className="chart">
        <ResponsiveContainer width="100%" height={230}>
          <BarChart data={data.monthly} margin={{ top: 18, right: 16, bottom: 4, left: -14 }}>
            ...
            <YAxis tick={{ fontSize: 22 }} unit="%" width={93} domain={[0, 100]} />
            <Tooltip
            ...
            <Bar dataKey="chance_pct" radius={[4, 4, 0, 0]} isAnimationActive={false}>
              <LabelList dataKey="chance_pct" position="top" fontSize={22} />
