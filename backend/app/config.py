...
# ---------- อายุของไฟล์ข้อมูลดิบ ----------
#
# ทุกรอบที่เก็บข้อมูลจะเขียนไฟล์ดิบไว้หนึ่งไฟล์ ไฟล์ละราวหนึ่งแสนสี่หมื่นไบต์
# เก็บวันละสี่รอบเป็นอย่างน้อย โฟลเดอร์จึงโตขึ้นเดือนละหลายสิบเมกะไบต์
# โดยไม่มีอะไรมาลบให้ ปล่อยไว้หลายปีคือกินพื้นที่เครื่องไปเรื่อย ๆ
#
# ไฟล์ดิบมีไว้กู้ข้อมูลกลับถ้าพบว่าโค้ดแปลงข้อมูลผิด ซึ่งเป็นเรื่องที่รู้ตัวได้เร็ว
# ถ้าผ่านไปเป็นเดือนแล้วยังไม่มีใครสังเกตเห็น แปลว่าตัวเลขในฐานข้อมูลใช้ได้แล้ว
# หนึ่งสัปดาห์จึงพอสำหรับย้อนดู และไม่ปล่อยให้โตไม่มีที่สิ้นสุด
#
# ตั้งเป็นศูนย์เมื่อไรคือปิดการลบ เก็บทุกไฟล์ไว้เหมือนเดิม
RAW_KEEP_DAYS = 7
