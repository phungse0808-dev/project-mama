...
class HivStatistic(SQLModel, table=True):
    """สถิติผู้ติดเชื้อเอชไอวีรายจังหวัด

    ใช้ประกอบการจัดลำดับพื้นที่ที่ควรเฝ้าระวังคุณภาพอากาศก่อน เพราะผู้มีภูมิคุ้มกัน
    บกพร่องเสี่ยงต่อการติดเชื้อทางเดินหายใจจากการรับฝุ่นมากกว่าประชากรทั่วไป

    ข้อกำหนดการใช้งานที่ต้องยึดอย่างเคร่งครัด
        1. เก็บได้เฉพาะระดับจังหวัดเท่านั้น ห้ามลงลึกกว่านี้ เช่นระดับอำเภอหรือตำบล
           เพราะยิ่งละเอียดยิ่งกลายเป็นการชี้เป้าและตีตราชุมชน
        2. เก็บได้เฉพาะตัวเลขรวมยอดที่หน่วยงานรัฐเผยแพร่เป็นสาธารณะแล้ว
           ห้ามนำเข้าข้อมูลรายบุคคลไม่ว่ากรณีใด
        3. ต้องระบุแหล่งที่มาและปีของข้อมูลทุกแถว เพื่อให้ตรวจสอบย้อนกลับได้

    ข้อมูลนี้ต่างจากข้อมูลอื่นในระบบตรงที่ไม่มี API ให้ดึงอัตโนมัติ
    ต้องนำเข้าจากรายงานที่เผยแพร่ด้วยสคริปต์ import_hiv.py
    """

    __table_args__ = (UniqueConstraint("province", "year", name="uq_hiv_province_year"),)

    id: int | None = Field(default=None, primary_key=True)
    province: str = Field(index=True)
    year: int = Field(index=True)  # พุทธศักราช

    cases: int | None = None  # จำนวนผู้ติดเชื้อที่ยังมีชีวิตอยู่
    rate_per_100k: float | None = None  # อัตราต่อประชากรแสนคน ใช้เทียบข้ามจังหวัดได้
    source: str  # ชื่อรายงานและหน่วยงานที่เผยแพร่
    note: str | None = None
