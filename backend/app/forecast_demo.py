...
เป็นเดโม ยังใช้งานจริงไม่ได้
    ข้อมูลที่ใช้เป็นค่าจริงทั้งหมด และสูตรผ่านการย้อนทดสอบกับข้อมูลที่ระบบเก็บเองแล้ว
    แต่ข้อมูลที่ใช้ทดสอบมีเพียงช่วงหน้าฝนสองเดือน ยังไม่ครอบคลุมหน้าหนาวที่ฝุ่นสูง
    ทิศทางของผลสภาพอากาศอ้างอิงงานวิจัย ส่วนขนาดของผลได้จากการปรับกับข้อมูลจริง
    หน้าเว็บต้องบอกทั้งความแม่นที่วัดได้และขอบเขตของข้อมูลที่ใช้วัด

ที่มาของโครงสูตร
    ไม่ได้ยกมาจากเอกสารใด ผู้จัดทำออกแบบเองเพื่อสาธิต
    แนวคิดคือใช้ค่าล่าสุดเป็นฐาน บวกแนวโน้ม แล้วปรับตามสภาพอากาศ
    ส่วนค่าตัวคูณปรับจากข้อมูลที่ระบบเก็บสะสมเอง ดูผลการวัดความแม่นใน ACCURACY
...
สูตร
    ...
    (2) C′ = C₀ + 0.3 × (C₀ − C₋₁)
        ...
        ตัวคูณ 0.3 ได้จากการปรับกับข้อมูลจริง ไม่ใช่ค่าที่กำหนดเอง
    ...
    (3) ตัวปรับตามสภาพอากาศของช่วงที่พยากรณ์ ค่อย ๆ ลดตามระดับ ไม่ใช้เกณฑ์ตัด
        ...
        f_ฝน     = 1 − 0.05 × R/100                          R = โอกาสฝนสูงสุด %  [2][3]
        f_ลม     = 1 − 0.00 × min(W, 30)/30                  W = ลมแรงสุด km/h    [4]
        f_ชื้น    = 1 − 0.02 × max(0, H − 60)/40               H = ความชื้นเฉลี่ย %  [5]
        f_ร้อน    = 1 − 0.00 × min(1, max(0, (T − 25)/10))     T = อุณหภูมิสูงสุด °C [5]
        ทิศทางของผลอ้างอิงงานวิจัยตามเลขในวงเล็บ ส่วนขนาดของผลได้จากการปรับกับข้อมูลจริง
        ลมกับอุณหภูมิได้ศูนย์ แปลว่าข้อมูลที่มีวัดผลของสองตัวนี้ไม่ได้ ไม่ใช่ว่าไม่มีผล

    (4) พรุ่งนี้ Ĉ₊₁ = max(0, C′ × f_ฝน × f_ลม × f_ชื้น × f_ร้อน)   ช่วง (t, t + 24 ชม.]

    (5) มะรืน คิดต่อจากพรุ่งนี้ C″ = Ĉ₊₁ + 0.3 × (Ĉ₊₁ − C₀) แล้วคูณตัวปรับด้วยอากาศของช่วง (t + 24, t + 48 ชม.]
...
# ค่าตัวคูณทั้งห้าตัวนี้ปรับจากข้อมูลที่ระบบเก็บสะสมเองจริง ไม่ใช่ค่าที่กำหนดเอง
#
# วิธีหา ใช้ค่าตรวจวัดรายชั่วโมงทั้งหมดในฐานข้อมูลสร้างชุดทดสอบแบบเดียวกับที่หน้าเว็บทำ
# คือเอาค่าเฉลี่ย 24 ชม. สองช่วงล่าสุดไปทายค่าเฉลี่ย 24 ชม. ถัดไป แล้วเทียบกับค่าจริง
# จากนั้นไล่ลองตัวคูณ 1,296 ชุด เลือกชุดที่ค่าคลาดเคลื่อนเฉลี่ยต่ำที่สุด
# สคริปต์อยู่ที่ scripts/calibrate_forecast.py รันซ้ำได้เมื่อมีข้อมูลมากขึ้น
#
# ค่าก่อนปรับคือ 0.5 / 0.20 / 0.10 / 0.05 / 0.05 ซึ่งผู้จัดทำกำหนดเอง
# และให้ค่าคลาดเคลื่อน 2.95 ซึ่งแย่กว่าการเดาด้วยค่าเมื่อวานที่ 1.94
# ชุดที่ปรับแล้วให้ 1.85 คือดีกว่าการเดา
TREND_WEIGHT = 0.3
# ลดได้สูงสุดเท่าไรเมื่ออากาศถึงระดับเต็ม
RAIN_MAX_CUT = 0.05
# ลมกับอุณหภูมิเป็นศูนย์ เพราะข้อมูลที่มีวัดผลของสองตัวนี้ไม่ได้
# คงตัวแปรไว้ในสูตรเพื่อให้ปรับกลับได้เมื่อมีข้อมูลหน้าหนาวที่ลมแรงกว่านี้
WIND_MAX_CUT = 0.0
HUMIDITY_MAX_CUT = 0.02
HEAT_MAX_CUT = 0.0
...
# ผลการย้อนทดสอบครั้งล่าสุด ใช้แสดงบนหน้าเว็บว่าสูตรนี้แม่นแค่ไหนเมื่อเทียบกับวิธีพื้นฐาน
#
# ตัวเลขนี้ได้จาก scripts/calibrate_forecast.py ไม่ได้คำนวณสดตอนเปิดหน้า
# เพราะต้องดึงพยากรณ์อากาศย้อนหลังของทุกจังหวัดซึ่งใช้เวลาหลายนาที
# เมื่อรันสคริปต์ใหม่ต้องมาแก้ตัวเลขชุดนี้ให้ตรงด้วย
ACCURACY = {
    "tested_at_th": "23 กันยายน 2569",
    "provinces": 66,
    "cases": 484,
    "readings": 71794,
    "period_th": "17 กรกฎาคม ถึง 23 กันยายน 2569",
    # เกณฑ์ที่คนทั่วไปเข้าใจ ทายระดับคุณภาพอากาศถูกกี่เปอร์เซ็นต์
    # แสดงคู่กับเลขของการเดาเสมอ เพราะช่วงที่ทดสอบเป็นฤดูฝนที่ค่าฝุ่นนิ่ง
    # ทายว่าดีมากไว้ก่อนก็ถูกเกือบหมดอยู่แล้ว ถ้าโชว์แต่ 88% จะทำให้เข้าใจว่าแม่นกว่าความจริง
    "level_hit_pct": 88,
    "level_hit_base_pct": 87,
    "rows": [
        {"name_th": "สูตรของระบบ ก่อนปรับ", "mae": 2.95, "bias": -2.84, "current": False},
        {"name_th": "เดาด้วยค่าเฉลี่ย 24 ชม. ล่าสุด", "mae": 1.94, "bias": 0.03, "current": False},
        {"name_th": "สูตรของระบบ หลังปรับ", "mae": 1.85, "bias": -0.50, "current": True},
    ],
    "note_th": (
        "ค่าคลาดเคลื่อนเฉลี่ยมีหน่วยเป็นไมโครกรัมต่อลูกบาศก์เมตร ยิ่งน้อยยิ่งแม่น · "
        "ข้อมูลที่ใช้ปรับเป็นช่วงหน้าฝนซึ่งค่าฝุ่นต่ำและนิ่ง ตัวคูณชุดนี้จึงเหมาะกับหน้าฝน "
        "เมื่อถึงหน้าหนาวที่ฝุ่นสูงและแกว่งต้องปรับใหม่ด้วยข้อมูลของช่วงนั้น · "
        "การปรับค่าจากข้อมูลชุดเดียวกับที่ใช้วัดผล ทำให้ตัวเลขดูดีกว่าความจริงเล็กน้อย"
    ),
}
...
#
# ใส่เฉพาะหน่วยงานที่เกี่ยวข้องกับระบบจริง ไม่ใส่หน่วยงานที่ยกมาเป็นตัวอย่างเฉย ๆ
# เพราะหน้านี้ตอบคำถามว่าเลขบนจอมาจากไหน ไม่ใช่ว่าโลกนี้มีใครทำเรื่องนี้บ้าง
#
# ข้อมูลของ CAMS และ NOAA ยืนยันจากเอกสารของหน่วยงานเอง ส่วนของไทยเป็นภาพรวม
# จากงานวิจัยและข่าว ยังไม่ใช่เอกสารทางการ ก่อนนำไปเขียนในเล่มต้องหาต้นทางยืนยันอีกชั้น
...
OUR_FORMULA = "Ĉ = (C₀ + 0.3 × (C₀ − C₋₁)) × f_ฝน × f_ชื้น"
...
FORMULA = [
    ...
    "C′ = C₀ + 0.3 × (C₀ − C₋₁)",
    "f_ฝน = 1 − 0.05 × โอกาสฝน/100",
    "f_ชื้น = 1 − 0.02 × max(0, ความชื้นเฉลี่ย − 60)/40",
    "ลมกับอุณหภูมิ ตัวคูณเป็นศูนย์ เพราะข้อมูลที่มีวัดผลของสองตัวนี้ไม่ได้",
